$(function () {
    ...
    new AjaxUpload('#importV1Button', {
        ...
        onComplete: function (file, r) {
            if (r.resultCode == 200) {
                swal("成功导入" + r.data + "条记录！", {icon: "success"});
                reload();
                return false;
            } else {
...
$('#importV2Button').click(function () {
    ...
    $.ajax({
        type: 'POST',
        dataType: "json",
        url: 'fly/users/importV2?fileUrl=' + fileUrl,
        contentType: "application/json; charset=utf-8",
        success: function (result) {
            checkResultCode(result.resultCode);
            console.log(result);
            if (result.resultCode == 200) {
                closeModal();
                reload();
                swal("成功导入" + result.data + "条记录！", {icon: "success"});
            }
            else {
                closeModal();
                swal(result.message, {icon: "warning"});
